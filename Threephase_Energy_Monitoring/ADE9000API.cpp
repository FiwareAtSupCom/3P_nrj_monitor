...
void ADE9000Class::SetupADE9000(void)
{
	 ...
	 SPI_Write_16(ADDR_EGY_TIME,0x0001);//ADE9000_EGY_TIME instead of 1 
	 SPI_Write_16(ADDR_EP_CFG,0x0021);		//Energy accumulation ON  ADE9000_EP_CFG instead of 
...
void ADE9000Class:: AccumulateActiveEnergy(volatile ActiveEnergyperH* ActiveEnergy)
{
  ...
  uint32_t DataC;
  //phase A
  DataA = int32_t ((SPI_Read_32(ADDR_AWATTHR_HI)) && (0x1FFFFFFF));//AWATTHR_HI goes to zero after read
	DataA = DataA<<13;
	DataA |= int32_t ((SPI_Read_32(ADDR_AWATTHR_LO)) && (0xFFF)); 
  ActiveEnergy->ActiveEnergy_A+=DataA;
  //phase B
  DataB = int32_t ((SPI_Read_32(ADDR_BWATTHR_HI)) && (0x1FFFFFFF));//BWATTHR_HI goes to zero after read
	DataB = DataB<<13;
	DataB |= int32_t ((SPI_Read_32(ADDR_BWATTHR_LO)) && (0xFFF));
  ActiveEnergy->ActiveEnergy_B+=DataB;
  //phase C
  DataC = int32_t ((SPI_Read_32(ADDR_CWATTHR_HI)) && (0x1FFFFFFF));//CWATTHR_HI goes to zero after read
	DataC = DataC<<13;
	DataC |= int32_t ((SPI_Read_32(ADDR_CWATTHR_LO)) && (0xFFF));
...
void ADE9000Class:: AccumulateReactiveEnergy(volatile ReactiveEnergyperH* ReactiveEnergy)
{
  ...
  uint32_t DataC;
  //phase A
  DataA = int32_t ((SPI_Read_32(ADDR_AVARHR_HI)) && (0x1FFFFFFF));//AWATTHR_HI goes to zero after read
	DataA = DataA<<13;
	DataA |= int32_t ((SPI_Read_32(ADDR_AVARHR_LO)) && (0xFFF)); 
  ReactiveEnergy->ReactiveEnergy_A+=DataA;
  //phase B
  DataB = int32_t ((SPI_Read_32(ADDR_BVARHR_HI)) && (0x1FFFFFFF));//BWATTHR_HI goes to zero after read
	DataB = DataB<<13;
	DataB |= int32_t ((SPI_Read_32(ADDR_BVARHR_LO)) && (0xFFF));
  ReactiveEnergy->ReactiveEnergy_B+=DataB;
  //phase C
  DataC = int32_t ((SPI_Read_32(ADDR_CVARHR_HI)) && (0x1FFFFFFF));//CWATTHR_HI goes to zero after read
	DataC = DataC<<13;
	DataC |= int32_t ((SPI_Read_32(ADDR_CVARHR_LO)) && (0xFFF));
  ReactiveEnergy->ReactiveEnergy_C+=DataC;
}
